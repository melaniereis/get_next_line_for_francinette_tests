...
{
	static char	*buffer[FOPEN_MAX]; // Static buffer for multiple file descriptors
	char		*line;
	...
	line = extract_line(buffer[fd]); // Extract line from buffer
	buffer[fd] = update_buffer(buffer[fd]); // Update buffer for next call
	return (line);
}
